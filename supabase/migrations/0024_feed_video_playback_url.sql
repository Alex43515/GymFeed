-- New Bunny uploads link a media_assets row to the post. The previous
-- feed_page implementation accidentally returned that asset's thumbnail_url
-- as video_url, so Home attempted to initialize a JPEG with the video player.
-- Post details were unaffected because they read posts.legacy_video_url.
...
revoke execute on function public.feed_page(integer, integer)
  from public, anon;
grant execute on function public.feed_page(integer, integer)
  to authenticated;
